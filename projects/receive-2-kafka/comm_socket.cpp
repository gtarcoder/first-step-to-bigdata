...
bool CommonSocket::SetReuseAddr(bool reuse){
    int opt = reuse;
    if(setsockopt(socket_fd_, SOL_SOCKET, SO_REUSEADDR, (const void*)&opt, sizeof(int)) < 0)
      return false;
    return true;
}

bool CommonSocket::SetReusePort(bool reuse){
    uint32_t opt = reuse;
    if(setsockopt(socket_fd_, SOL_SOCKET, SO_REUSEPORT, (char*)&opt, sizeof(opt)) < 0){
       perror("set socket reuse port failed!\n");
       return false;
    }
    return true;
}
